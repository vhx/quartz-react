...
const StatelessCarousel = ({ currentSlide, height, isReady, onSlideChange, priorSlide, slides }) => {
  const hasMoreThanOneSlide = slides.length > 1;
  const gotoPrev = () => onSlideChange(prev(slides.length, currentSlide), currentSlide);
  const gotoNext = () => onSlideChange(next(slides.length, currentSlide), currentSlide);
  return (
    <div className='carousel' style={{ height: `${height}px` }}>
      <div>
        { slides.map((Slide, i) => <Slide key={i} isActive={currentSlide === i} wasActive={priorSlide === i} isReady={isReady} />) }
      </div>
      <If condition={hasMoreThanOneSlide}>
        ...
        <div className='coins'>
          { slides.map((Slide, i) => <button key={i} onClick={() => onSlideChange(i, currentSlide)} disabled={currentSlide === i} className={currentSlide === i ? 'coin active' : 'coin'} />) }
        </div>
      </If>
    </div>
  );
...
class Carousel extends Component {
  ...
  setSlide(currentSlide, priorSlide) {
    this.setState({ currentSlide, priorSlide, isReady: true });
  }
  render() {
    return (
      <StatelessCarousel currentSlide={this.state.currentSlide} priorSlide={this.state.priorSlide} onSlideChange={this.setSlide} height={700} isReady={this.state.isReady} slides={this.props.slides} />
    );
  }
}
